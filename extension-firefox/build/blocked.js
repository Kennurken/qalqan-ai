// Qalqan AI — blocked.js
// Standalone block page: reads threat data from URL hash, renders block UI

(async () => {
  // ── Parse data from URL hash ──────────────────────────────────────────────
  let d = {
    url: "", score: 0, type: "unknown", source: "unknown",
    detail_kk: "", detail_ru: "", detail_en: "", indicators: [],
  };
  try {
  ...
  let lang = "kk";
  try {
    const s = await chrome.storage.local.get("qalqan_lang");
    if (s.qalqan_lang) lang = s.qalqan_lang;
  } catch {}
  ...
  const I18N = {
    kk: {
      ...
      threatScore: "Қауіп деңгейі", threatType: "Қауіп түрі",
      detectedThreat: "Анықталған қауіп", dataSource: "Дерек көзі",
      back: "← Қауіпсіз жерге қайту",
      appeal: "Қателік деп ойласаңыз — кеңейтімдегі «Апелляция» батырмасын басыңыз",
      gambling_hotline: "Лудомания — емделетін ауру",
      gambling_help: "Тегін психологиялық көмек:",
    },
    ru: {
      brand: "QALQAN AI — КИБЕР ЩИТ",
      blocked: "САЙТ ЗАБЛОКИРОВАН",
      subtitle: "Qalqan AI обнаружил угрозу и защитил вас",
      threatScore: "Уровень угрозы", threatType: "Тип угрозы",
      detectedThreat: "Обнаруженная угроза", dataSource: "Источник данных",
      back: "← Вернуться в безопасное место",
      appeal: "Считаете это ошибкой? Нажмите «Апелляция» в расширении",
      gambling_hotline: "Лудомания — это болезнь, которую лечат",
      gambling_help: "Бесплатная психологическая помощь:",
    },
    en: {
      brand: "QALQAN AI — CYBER SHIELD",
      blocked: "SITE BLOCKED",
      subtitle: "Qalqan AI detected a threat and protected you",
      threatScore: "Threat Level", threatType: "Threat Type",
      detectedThreat: "Detected Threat", dataSource: "Data Source",
      back: "← Return to Safety",
      appeal: "Think this is an error? Click 'Appeal' in the extension",
      gambling_hotline: "Gambling addiction is treatable",
      gambling_help: "Free helpline (Kazakhstan):",
    },
  };
  const TYPES = {
    kk: { phishing: "Фишинг", malware: "Зиянды бағдарлама", pyramid: "Қаржылық пирамида", scam: "Алаяқтық", gambling: "Лицензиясыз құмар ойын", fake_shop: "Жалған дүкен", social_engineering: "Әлеуметтік инженерия", suspicious_infrastructure: "Күдікті инфрақұрылым", unknown: "Белгісіз қауіп" },
    ru: { phishing: "Фишинг", malware: "Вредоносное ПО", pyramid: "Финансовая пирамида", scam: "Мошенничество", gambling: "Нелицензированный гемблинг", fake_shop: "Поддельный магазин", social_engineering: "Социальная инженерия", suspicious_infrastructure: "Подозрительная инфраструктура", unknown: "Неизвестная угроза" },
    en: { phishing: "Phishing", malware: "Malware", pyramid: "Financial Pyramid", scam: "Scam", gambling: "Unlicensed Gambling", fake_shop: "Fake Shop", social_engineering: "Social Engineering", suspicious_infrastructure: "Suspicious Infrastructure", unknown: "Unknown Threat" },
  };
  const SOURCES = {
    ...
    kz_intel: "KZ Intel", kz_impersonation: "KZ Brand Protection",
    offline_pyramid: "Offline: Pyramid DB", offline_gambling: "Offline: Gambling DB",
    offline_phishing: "Offline: Phishing DB", offline_scam: "Offline: Scam DB",
    url_features: "URL Feature Analysis", quick_check: "Qalqan Quick Check",
  };

  const t      = I18N[lang]  || I18N.kk;
  const types  = TYPES[lang] || TYPES.kk;
  const score  = Number(d.score || 0);
  const ttKey  = d.type   || "unknown";
  const srcKey = d.source || "unknown";
  const reason = d[`detail_${lang}`] || d.detail_kk || d.detail || "";
  ...
  const srcName  = esc(SOURCES[srcKey] || srcKey);

  let blockedDomain = "";
  try { blockedDomain = new URL(d.url).hostname.replace("www.", ""); } catch {}
  ...
  const tagHTML = (d.indicators || []).slice(0, 6)
    .map(i => `<span class="qtag">${esc(i)}</span>`).join("");
  ...
  const css = `
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    #qcenter{position:fixed;inset:0;z-index:10;display:flex;align-items:center;justify-content:center;padding:16px;}
    #qcard{
      width:100%;max-width:540px;max-height:calc(100vh - 32px);overflow-y:auto;scrollbar-width:none;
      background:rgba(13,20,38,0.88);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);
      border:1px solid rgba(${scoreRgb},0.28);border-radius:24px;padding:36px 32px 28px;
      position:relative;
    ...
    #qcard::after{content:'';position:absolute;inset:0;pointer-events:none;
      background:radial-gradient(ellipse 70% 35% at 50% 0%,rgba(${scoreRgb},0.07),transparent);}
    #qheader{display:flex;flex-direction:column;align-items:center;text-align:center;margin-bottom:28px;}
    ...
    #qshield-icon{
      ...
      background:rgba(${scoreRgb},0.1);border:1px solid rgba(${scoreRgb},0.3);
      display:flex;align-items:center;justify-content:center;color:${scoreHex};
      position:relative;z-index:1;animation:qPulse 2.5s ease-in-out infinite;
    }
    ...
    .qm{background:rgba(${scoreRgb},0.06);border:1px solid rgba(${scoreRgb},0.12);border-radius:14px;padding:14px 16px;}
    ...
    #qreason{background:rgba(0,0,0,0.22);border:1px solid rgba(255,255,255,0.05);
      border-radius:14px;padding:16px;margin-bottom:14px;}
    .qr-label{font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#64748b;margin-bottom:8px;
      font-family:'SF Mono','Fira Code',monospace;display:flex;align-items:center;gap:6px;}
    .qr-text{font-size:13px;color:#94a3b8;line-height:1.65;}
    .qtags{display:flex;flex-wrap:wrap;gap:5px;margin-top:10px;}
    .qtag{font-size:10px;font-weight:600;padding:3px 9px;border-radius:20px;letter-spacing:0.3px;
    ...
    #qback{
      width:100%;background:rgba(0,212,255,0.08);color:#00d4ff;
      border:1.5px solid rgba(0,212,255,0.5);padding:15px 28px;border-radius:12px;
      cursor:pointer;font-weight:700;font-size:14px;
      font-family:'SF Mono','Fira Code','Consolas',monospace;
      letter-spacing:1.5px;text-transform:uppercase;
      box-shadow:0 0 20px rgba(0,212,255,0.12);transition:all 0.2s;position:relative;overflow:hidden;
    }
    #qback::before{content:'';position:absolute;top:0;left:-100%;width:60%;height:100%;
      background:linear-gradient(90deg,transparent,rgba(0,212,255,0.1),transparent);transition:left 0.4s;}
    #qback:hover{background:rgba(0,212,255,0.14);box-shadow:0 0 32px rgba(0,212,255,0.25),inset 0 0 20px rgba(0,212,255,0.04);border-color:#00d4ff;}
    ...
    @keyframes qSlideIn{from{opacity:0;transform:translateY(36px) scale(0.95);}to{opacity:1;transform:translateY(0) scale(1);}}
    @keyframes qPulse{0%,100%{box-shadow:0 0 0 0 rgba(${scoreRgb},0.5);}50%{box-shadow:0 0 0 14px rgba(${scoreRgb},0);}}
    @keyframes qSpin{from{transform:rotate(0deg);}to{transform:rotate(360deg);}}
    @media(max-width:500px){#qcard{padding:24px 16px 20px;}#qtitle{font-size:18px;}.qm-num{font-size:26px;}}
  `;

  const gamblingInfo = ttKey === "gambling" ? `
    <div style="margin-top:14px;padding:10px 14px;background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.25);border-radius:8px;text-align:center;">
      <div style="font-size:11px;color:#22c55e;font-weight:600;margin-bottom:4px;display:flex;align-items:center;gap:5px;justify-content:center;">
        ...
        ${esc(t.gambling_hotline)}
      </div>
      <div style="font-size:11px;color:#94a3b8;">${esc(t.gambling_help)} <strong style="color:#f8fafc;">8-800-080-88-87</strong></div>
    </div>` : "";
      ...
      <div id="qcard">
        ...
        <div id="qmetrics">
          <div class="qm">
            ...
            <div class="qarc-wrap">
              <div class="qarc-track"><div class="qarc-fill" id="qarc-fill"></div></div>
            </div>
          </div>
          <div class="qm">
        ...
        <div id="qreason">
          ...
          <div class="qr-text">${esc(reason)}</div>
  ...
  document.addEventListener("keydown", e => {
    if (e.key === "Escape") document.getElementById("qback").click();
  });

  requestAnimationFrame(() => {
    const arc = document.getElementById("qarc-fill");
    if (arc) requestAnimationFrame(() => { arc.style.width = score + "%"; });
  });
})();
