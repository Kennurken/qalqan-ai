...
async function scan(dom){
  ...
  try{
    ...
    const dot=d.verdict==='DANGEROUS'?'#f7768e':d.verdict==='SUSPICIOUS'?'#e0af68':'#9ece6a';
    const word=d.verdict==='DANGEROUS'?T('danger'):d.verdict==='SUSPICIOUS'?T('susp'):T('clean');
    $('#vv').innerHTML='<svg class="qi" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="7" fill="'+dot+'"/></svg> '+word+' · '+T('risk')+' '+d.risk_score+'/100';
