...
async def handle_start(chat_id: int, first_name: str = ""):
    text = (
        ...
        f"  /watch &lt;домен&gt; — брендіңізді бақылау (клон-домендер)\n"
        f"  /subscribe — апталық қауіп-дайджест\n"
        f"  /leak — құпиясөз утечкасын тексеру\n"
        f"  /sos — 🆘 алдап кетті ме? не істеу керек\n"
...
async def handle_help(chat_id: int):
    text = (
        ...
        "  /watch kaspi.kz — брендті бақылау: жаңа клон-домен/SSL-серт шықса, жеке хабарлаймыз\n"
        "  /watchlist — бақылаудағы брендтер · /unwatch — тоқтату\n"
        "  /subscribe — апталық қауіп-дайджест (жексенбі сайын)\n"
        "  /leak — құпиясөз утечкада ма (HIBP)\n"
        "  /sos — 🆘 алдап кетті ме? 1477, әрекет жоспары\n"
